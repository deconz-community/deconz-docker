...
- The deCONZ process no longer runs as root, so the container path for the deCONZ files changed from `/root/.local/share/dresden-elektronik/deCONZ` to `/opt/deCONZ`.
